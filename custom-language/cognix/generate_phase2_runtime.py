...
from runtime.function import Callable, CognixFunction, ReturnException
...
class Interpreter(Visitor):
    ...
    def visit_function_declaration(self, stmt):
        function = CognixFunction(stmt, self.environment)
        self.environment.define(stmt.name.lexeme, function)

    def visit_agent_declaration(self, stmt):
        methods = {}
        for task in stmt.tasks:
            methods[task.name.lexeme] = CognixFunction(task, self.environment)
        agent = CognixAgent(stmt.name.lexeme, methods)
...
with open(os.path.join(base_dir, 'examples', 'agent.cgx'), 'w', encoding='utf-8') as f:
