...
def replace_in_file(filepath):
    # skip self
    if os.path.basename(filepath) in ('rename.py', 'rename_ext.py'):
        return
    ...
    new_content = content
    # Replacements
    new_content = re.sub(r'\.cgx', '.agx', new_content)
...
def rename_files_and_directories(root_dir):
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir, topdown=False):
        ...
        for filename in filenames:
            if filename in ('rename.py', 'rename_ext.py'):
                continue
            if filename.endswith('.cgx'):
                new_name = filename[:-4] + '.agx'
